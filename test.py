...
from adda import s_encoder, t_encoder, classifier,discriminator, build_ad_loss_v2, eval, build_ad_loss,concat_ad_loss_2
from config import Config
import numpy as np
from utils import loglikelihood, fresh_dir
config = Config()
...
#source_train_img = normalization(source_train_img)
source_train_lab = s_label_train
source_test_img = np.transpose(test_source,[0,2,3,1])
#source_test_img = normalization(source_test_img)
source_test_lab = s_label_test
target_train_img = np.transpose(train_target,[0,2,3,1])
#target_train_img = normalization(target_train_img)
target_train_lab = t_label_train
target_test_img = np.transpose(test_target,[0,2,3,1])
#target_test_img = normalization(target_test_img)
print(source_train_img.shape)
print(target_test_img.shape)
target_test_lab = t_label_test


# source_test_img = np.load('cropped_test_imgs.npy')
# source_test_lab = np.load('cropped_test_labels.npy')
# source_test_lab = np.argmax(source_test_lab,1)
...
def set_log_info(name):
    ...
    return logger
logger = set_log_info('ram_adda.log')
...
def step1(logdir = './source_model'):
    ...
    # logits_te = classifier(s_encoder(x_te, reuse=True), reuse=True)
...
#        saver_source.restore(sess, "./source_model/encoder/encoder.ckpt-105")
        saver_cls = tf.train.Saver(var_cls)
 #       saver_cls.restore(sess, "./source_model/classifier/classifier.ckpt-105")

        for i in range(config.epoch):
            num_batches = target_train_img.shape[0] // config.batch_size
            # svhn = np.concatenate(
            #     (np.reshape(source_train_img, [-1, 32 * 32 * 3]), np.expand_dims(source_train_lab, 1)), 1)
            # mnist = np.concatenate(
            #     (np.reshape(target_train_img, [-1, 32 * 32 * 3]), np.expand_dims(target_train_lab, 1)), 1)
            # svhn_shuffle = np.random.permutation(svhn)
            # mnist_shuffle = np.random.permutation(mnist)
            # source_shuffle_train_img = np.reshape(svhn_shuffle[:, :-1], [-1, 32, 32, 3])
            # source_shuffle_train_lab = svhn_shuffle[:, -1]
            # target_shuffle_train_img = np.reshape(mnist_shuffle[:, :-1], [-1, 32, 32, 3])
            # target_shuffle_train_lab = mnist_shuffle[:, -1]
			#
            ...
            for j in range(num_batches):
                ...
                # if svhn_train_img.shape[0]<config.batch_size or mnist_train_img.shape[0]<config.batch_size:
                #     print(j)
                #
                #     break
                if j and j % 10 == 0:
...
def step2(source_dir = './source_model'):
    ...
    #t_acc = eval(t_logits, target_labels_ph)
    # g_loss, d_loss = build_ad_loss(disc_s, disc_t)
    # d_loss_source = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_s,
    #                                                                        labels=tf.ones_like(
    #                                                                            disc_s)))  # log(D(x))
    # d_loss_target = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_t,
    #                                                                        labels=tf.zeros_like(
    #                                                                            disc_t)))  # log(1-D(G(z)))
	#
    # d_loss = d_loss_source + d_loss_target  # log(D(x)) + log(1-D(G(z)))
    # g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_t,
    #                                                                             labels=tf.ones_like(
    #                                                                                 disc_t)))  # log(D(G(z)))

    # adversary_ft = tf.concat([s_feature, t_feature], 0)
    # source_adversary_label = tf.zeros([tf.shape(s_feature)[0]], tf.int64)
    # target_adversary_label = tf.ones([tf.shape(t_feature)[0]], tf.int64)
    # adversary_label = tf.concat(
    #     [source_adversary_label, target_adversary_label], 0)
    # adversary_logits = discriminator(
    #     adversary_ft, reuse=False)

	# d_loss = tf.losses.sparse_softmax_cross_entropy(
     #    1 - adversary_label, adversary_logits)
	# g_loss = tf.losses.sparse_softmax_cross_entropy(
     #    target_adversary_label, adversary_logits[config.batch_size:, :])
    ...
    # source_vars = collect_vars('s_encoder',prepend_scope='s_encoder')
    # target_vars = collect_vars('t_encoder',prepend_scope='s_encoder')

    #logits_t = classifier(t_feature,reuse=True,trainable=False)
    ...
    # var_source = [k for k in all_vars if k.name.startswith("s_encoder")]
    # var_target = [k for k in all_vars if k.name.startswith("t_encoder")]
    ...
    # optim_g = tf.train.AdamOptimizer(1e-4, beta1=0.5, beta2=0.999).minimize(loss, var_list=var_target)
    optim_g = tf.train.AdamOptimizer(1e-4,beta1=0.5, beta2=0.999).minimize(g_loss, var_list=var_target,global_step=global_step)
    # optim_g = tf.train.RMSPropOptimizer(learning_rate=0.0001).minimize(loss,var_list=var_target)


    # optim_d = tf.train.AdamOptimizer(1e-4, beta1=0.5, beta2=0.999).minimize(d_loss, var_list=var_dis)
    # optim_d = tf.train.AdamOptimizer(2e-4,beta1=0.5,beta2=0.999).minimize(d_loss, var_list=var_dis,global_step=global_step)
    # optim_d = tf.train.RMSPropOptimizer(1e-4).minimize(d_loss, var_list=var_dis, global_step=global_step)
    # clip_d_op = [var.assign(tf.clip_by_value(var, -0.01, 0.01)) for var in var_dis]
    ...
    with tf.Session() as sess:
        ...
        sess.run(tf.global_variables_initializer())
        # saver_source = tf.train.Saver(var_source)
        # saver_source.restore(sess, "./source_model/encoder/encoder.ckpt-30")
        # saver_cls = tf.train.Saver(var_cls)
        # saver_cls.restore(sess, "./source_model/classifier/classifier.ckpt-30")
        # saver_target = tf.train.Saver(var_target)
        # saver_target.restore(sess, "./target_pretrain/encoder.ckpt")
        ...
        for i in range(config.epoch):
            ...
            for j in range(num_batches):
                ...
                #
                #     break
                # svhn_train_img = np.tile(svhn_train_img, [config.M, 1, 1, 1])
                # svhn_train_lab = np.tile(svhn_train_lab, [config.M])
                # mnist_train_img = np.tile(mnist_train_img, [config.M, 1, 1, 1])
                # mnist_train_lab = np.tile(mnist_train_lab, [config.M])
